...
class Userinfo extends Command {
    ...
    async run (message, args, data) {
        ...
        if(member){
            ...
            .addField(message.language.get("USERINFO_FIELDS")[10], (member.roles.size > 10 ? member.roles.toString().slice(0, 9).map((r) => r).join(", ")+message.language.get('USERINFO_MORE_ROLES', member.roles.size - 10) : (member.roles.size < 1) ? message.language.get("USERINFO_NO_ROLE") : member.roles.map((r) => r).join(", ")));
        }
