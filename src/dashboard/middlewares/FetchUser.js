...
module.exports = async (req, _res, next) => {
    ...
    sessionUser.guilds.forEach(async guild => {
        ...
        const guildJSON = await req.client.broadcastEval(
            `const guild = this.guilds.cache.get('${guild.id}'); if(guild) guild.toJSON();`,
            true
        );
        guild.baseInviteURL = `https://discordapp.com/oauth2/authorize?client_id=${
            req.client.user.id
        }&scope=bot&permissions=2146958847&guild_id=${
            guild.id
        }&response_type=code&redirect_uri=${encodeURIComponent(
            req.client.config.dashboard.baseURL + "/auth/callback"
        )}&state`;
        guild.manageURL = guildJSON
            ? `/guild/manage/${guild.id}`
            : `${baseInviteURL}=manage:${guild.id}`;
    ...
    sessionUser.manageableGuilds = sessionUser.guilds.filter(
    ...
    const member = await req.client.guilds.cache
        .get(req.config.support.id)
        .members.fetch(sessionUser.id)
        .catch(() => {});
