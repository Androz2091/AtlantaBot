...
module.exports = {
    ...
    init(client) {
        setInterval(async () => {
            let muted = await client.membersData.find({
                "mute.muted": true,
                "mute.endDate": { $lte: Date.now() }
            });
            muted.forEach(async memberData => {
                let guild = client.guilds.get(memberData.guildID);
                if (!guild) return;
                let member =
                    guild.members.get(memberData.id) ||
                    (await guild.members.fetch(memberData.id).catch(err => {
                        memberData.mute = {
                            muted: false,
                            endDate: null,
                            case: null
                ...
                guild.channels.forEach(channel => {
                    let permOverwrites = channel.permissionOverwrites.get(
                        member.id
                    );
