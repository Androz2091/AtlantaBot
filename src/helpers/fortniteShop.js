...
Canvas.registerFont(
    resolve("./assets/fonts/Burbank-Big-Condensed-Bold-Font.otf"),
    { family: "Burbank" }
);
...
async function deleteOldFiles() {
    langList.forEach(lang => {
        let dateNow = new Date(),
            delDate = new Date(
                dateNow.setTime(dateNow.getTime() - 7 * 86400000)
            ),
 ...
 */
async function writeImage(client) {
        ...
        langList.forEach(async lang => {
            const language = new (require(`../languages/${lang}.js`))(),
                ...
                fortniteClient = new fortnite.Client({
                    fnbrToken: client.config.apiKeys.fortniteFNBR
                });

            let shop = await fortniteClient.fnbrShop(),
                dateShop = new Date(shop.data.date),
                dailyHeight =
            ...
            let background = await Canvas.loadImage(
                "./assets/img/fortnite/shop/background.png"
            );
            ...
            ctx.fillText(
                language.get("FORTNITESHOP_HEADER"),
                canvas.width / 2,
                71
            );
            ...
            if (shop.data.daily.length < 9 && shop.data.featured.length < 9) {
                ...
                // Draw daily
                ctx.fillText(
                    language.get("FORTNITESHOP_DAILY"),
                    canvas.width - 447,
                    185
                );
            }
            // Draw footter
            ctx.fillText(
                client.config.embed.footer + " - fnbr.co",
                canvas.width / 2,
                canvas.height - 18
            );
            ...
            if (shop.data.daily.length < 9 && shop.data.featured.length < 9) {
                    ...
                    if (i & 1) {
                        if (shop.data.featured[i].images.featured) {
                            if (fs.existsSync(card)) {
                                ...
                                let background = await Canvas.loadImage(
                                    "./assets/img/fortnite/shop/card_common.png"
                                );
                                ctx.drawImage(
                                    background,
                                    313,
                            ...
                            let itemDesc = await Canvas.loadImage(
                                "./assets/img/fortnite/shop/card_itemdesc.png"
                            );
                            ctx.drawImage(
                                itemDesc,
                                313 + 3,
                            ...
                            let price = shop.data.featured[i].price.replace(
                                    /[,]/gi,
                                    ""
                                ),
                ...
                for (let i = 0; i < shop.data.daily.length; i++) {
                    ...
                    if (i & 1) {
                        if (shop.data.daily[i].images.daily) {
                            ...
                            let price = shop.data.daily[i].price.replace(
                                    /[,]/gi,
                                    ""
                                ),
                                vbuck = await Canvas.loadImage(
                ...
                let i1Featured = Math.ceil(shop.data.featured.length / 3),
                    i2Featured = Math.ceil(
                        (shop.data.featured.length - i1Featured) / 2
                    ),
                    ...
                    i3Daily = Math.ceil(
                        shop.data.daily.length - (i1Daily + i2Daily)
                    );
                ...
                for (let i = 0; i < i1Daily; i++) {
                    ...
                    if (shop.data.daily[i].images.daily) {
                        ...
                        let background = await Canvas.loadImage(
                            `./assets/img/fortnite/shop/card_${shop.data.daily[i].rarity}.png`
                        );
                ...
                for (
                    let i = i1Daily + i2Daily;
                    i < i1Daily + i2Daily + i3Daily;
                    i++
                ) {
                    let card = `./assets/img/fortnite/shop/card_${shop.data.daily[i].rarity}.png`;
                    if (shop.data.daily[i].images.daily) {
...
async function init(client) {
    ...
    new CronJob(
        ...
        async function() {
            if (
                !client.config.apiKeys.fortniteFNBR ||
                client.config.apiKeys.fortniteFNBR === ""
            )
                return;
            await writeImage(client);
            client.guilds.forEach(async guild => {
                let guildData = await client.findOrCreateGuild({
                    id: guild.id
                });
                ...
                if (guildData.plugins.fortniteshop && fs.existsSync(path)) {
                    let fnChannel = client.channels.get(
                        ...
                        let attachment = new Discord.MessageAttachment(
                            ...
                            embed = new Discord.MessageEmbed()
                                .setAuthor(
                                    language.get(
                                        "FORTNITESHOP_TITLE",
                                        language.printDate(new Date(Date.now()))
                                    ),
                                    client.user.displayAvatarURL()
                                )
                                .attachFiles(attachment)
                                .setImage(`attachment://${getFileName()}.png`)
