    ...
    constructor() {
		this.language = {
			...
			UTILS: {
				...
				NO_REASON_PROVIDED: "no se ha dado ninguna razón",
				UNDEFINED: "Indefinido",
				PLEASE_WAIT: `${e.loading} | Por favor, espere un momento...`,
				PREFIX: "Prefijo",
				CUSTOM_COMMANDS: "Pedidos personalizados",
				ANDMORE: "**y más**",
				TITLE: "Título",
				AUTHOR: "Autor",
				SIGN_OUT: "Desconexión",
			...
			/* DBL VOTES */
			VOTE_THANKS: (user) => `:arrow_up: Hola ${user.toString()}, gracias por votar!\nSu recompensa: 40 créditos!`,
			VOTE_LOGS: (user) => `:arrow_up: **${user.tag}** (\`${user.id}\`) votó por **Atlanta** y ganó **40** créditos, ¡gracias!\nhttps://discordbots.org/bot/557445719892688897/vote`,

			/* DEFAULT MESSAGES */
			NO_DESCRIPTION_PROVIDED: "No hay descripción",
			NO_USAGE_PROVIDED: "Ningún uso",
			NO_EXAMPLE_PROVIDED: "No se han dado ejemplos",

			// ERROR MESSAGES

			ERR_COMMAND_DISABLED: `${e.error} | Este comando está actualmente desactivado!`,
			ERR_OWNER_ONLY: `${e.error} | Sólo ${c.owner.name} puede hacer estas órdenes!`,
			ERR_INVALID_CHANNEL: `${e.error} | Por favor, mencione una feria válida!`,
			ERR_INVALID_ROLE: `${e.error} | Por favor, mencione un rol válido!`,
			ERR_INVALID_MEMBER: `${e.error} | Por favor, mencione un miembro válido!`,
			ERR_INVALID_NUMBER: (nan) => `${e.error} | \`${nan}\` no es un número válido!`,
			ERR_INVALID_NUMBER_MM: (min, max) => `${e.error} Por favor, indique un número válido entre ${min} y ${max}!`,
			ERR_INVALID_TIME: `${e.error} | Debe introducir un tiempo válido! Unidades válidas: \`s\`, \`m\`, \`h\`, \`d\`, \`w\`, \`y\``,
			ERR_INVALID_ID: `${e.error} | Por favor, introduzca una identificación válida!`,

			ERR_MISSING_BOT_PERMS: (perms) => `${e.error} | Necesito los siguientes permisos para realizar este comando: \`${perms}\``,
			ERR_MISSING_MEMBER_PERMS: (perm) => `${e.error} | No tiene los permisos necesarios para realizar este comando (\`${perm}\`)`,
			ERR_NOT_NSFW: `${e.error} | Debes ir a un salón que permita a la NSFW escribir este comando!`,
			ERR_GUILDONLY: `${e.error} | Este comando sólo está disponible en un servidor!`,
			ERR_UNAUTHORIZED_CHANNEL: (channel) => `${e.error} | Los pedidos están prohibidos en ${channel} !`,
			ERR_BAD_PARAMETERS: (cmd, prefix) => `${e.error} | Por favor, compruebe los parámetros del pedido. Mira los ejemplos escribiendo \`${prefix}help ${cmd}\` !`,
			ERR_ROLE_NOT_FOUND: (role) => `${e.error} | No se ha encontrado ningún rol con \`${role}\` !`,
			ERR_CHANNEL_NOT_FOUND: (channel) => `${e.error} | No se ha encontrado ninguna sala de estar con \`${channel}\``,
			ERR_YES_NO: `${e.error} | Debes responder "sí" o "no"!`,
			ERR_EVERYONE: `${e.error} | No se le permite mencionar a todos o aquí en las órdenes.`,
			ERR_BOT_USER: `${e.error} | Este usuario es un bot!`,
			ERR_GAME_ALREADY_LAUNCHED: `${e.error} | Una parte ya está en progreso en este servidor!`,
			ERR_A_GAME_ALREADY_LAUNCHED: `${e.error} | Debido a los retrasos y errores debidos a las palabras clave y al número, es imposible lanzar dos juegos al mismo tiempo, incluso si están en dos servidores diferentes.\nActualmente hay un juego en progreso en otro servidor, así que espera unos minutos y vuelve a intentarlo. \nLo sentimos, pero la gente estaba abusando de este comando al enviarlo a muchos servidores,
			ERR_OCCURENCED: `${e.error} | Se ha producido un error, inténtelo de nuevo en unos minutos.`,
			ERR_CMD_COOLDOWN: (seconds) => `${e.error} | Debe esperar **${seconds}** segundo(s) para poder realizar este pedido de nuevo!`,
