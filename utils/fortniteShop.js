    ...
    new CronJob("0 1 2 * * *", async function() {
        ...
        client.guilds.forEach(async (guild) => {
            ...
            if(guildData.plugins.fortniteshop && fs.existsSync(path)) {
                ...
                if(fnChannel) {
                    let attachment = new Discord.MessageAttachment(path, `${getFileName()}.png`),
                    embed = new Discord.MessageEmbed()
                    .setAuthor(language.get("FORTNITESHOP_TITLE", language.printDate(new Date(Date.now()))), client.user.displayAvatarURL())
