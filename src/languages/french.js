...
let { Emojis: e } = require("../utility/Constants");
...
module.exports = class {
    constructor() {
        this.language = {
            ...
            /* DBL VOTES */
            VOTE_THANKS: user =>
                `:arrow_up: Bonjour ${user.toString()}, merci de voter !\nVotre récompense : 40 crédits !`,
            VOTE_LOGS: user =>
                `:arrow_up: **${user.tag}** (\`${user.id}\`) a voté pour **Atlanta** et a gagné **40** crédits, merci !\nhttps://discordbots.org/bot/557445719892688897/vote`,
            ...
            ERR_INVALID_NUMBER: nan =>
                `${e.ERROR} | \`${nan}\` n'est pas un nombre valide !`,
            ERR_INVALID_NUMBER_MM: (min, max) =>
                `${e.ERROR} Veuillez indiquer un nombre valide entre ${min} et ${max} !`,
            ...
            ERR_MISSING_BOT_PERMS: perms =>
                `${e.ERROR} | J'ai besoin des permissions suivantes pour effectuer cette commande : \`${perms}\``,
            ERR_MISSING_MEMBER_PERMS: perm =>
                `${e.ERROR} | Vous n'avez pas les permissions nécessaires pour effectuer cette commande (\`${perm}\`)`,
            ...
            ERR_UNAUTHORIZED_CHANNEL: channel =>
                `${e.ERROR} | Les commandes sont interdites dans ${channel} !`,
            ERR_BAD_PARAMETERS: (cmd, prefix) =>
                `${e.ERROR} | Veuillez vérifier les paramètres de la commande. Regardez les exemples en tapant \`${prefix}help ${cmd}\` !`,
            ERR_ROLE_NOT_FOUND: role =>
                `${e.ERROR} | Aucun rôle trouvé avec \`${role}\` !`,
            ERR_CHANNEL_NOT_FOUND: channel =>
                `${e.ERROR} | Aucun salon trouvé avec \`${channel}\``,
            ...
            ERR_CMD_COOLDOWN: seconds =>
                `${e.ERROR} | Vous devez attendre **${seconds}** seconde(s) pour pouvoir de nouveau effectuer cette commande !`,
            ...
            // Content
            PING: ms =>
                `${e.SUCCESS} | Pong ! Ma latence est de \`${ms}\` ms !`,

            /* HELP COMMAND */

            // Utils
            HELP_DESCRIPTION:
                "Affiche l'aide des commandes ou l'aide d'une commande en particulier",
            HELP_USAGE: "help (commande)",
            HELP_EXAMPLES: "$help\n$help ping",
            // Errors
            HELP_ERR_NOT_FOUND: cmd =>
                `${e.ERROR} | Commande \`${cmd}\` introuvable !`,
            HELP_ERR_CMD_CUSTOMIZED: cmd =>
                `${e.ERROR} | La commande ${cmd} ne dispose pas d'aide car elle est personnalisée.`,
            // Content
            HELP_EDESCRIPTION: prefix =>
            ...
            // Utils
            FINDWORDS_DESCRIPTION:
                "Lance une partie de findwords, un jeu ou vous devez trouver des mots !",
            FINDWORDS_USAGE: "findwords",
            FINDWORDS_EXAMPLES: "$findwords",
            // Errors
            FINDWORDS_ERR_INVALID_WORD: member =>
                `${e.ERROR} | ${member} ton mot est invalide !`,
            ...
            FINDWORDS_QUESTION: word =>
                `${e.WARN} | 20 secondes pour trouver un mot contenant "**${word}**" !`,
            FINDWORDS_CONGRATS: winner =>
                `${e.SUCCESS} | Bravo <@${winner}> ! Ton mot est valide et tu as été le plus rapide !`,
            FINDWORDS_STATS: (username, games, time, number, members) =>
                `:tada: | ${username} a gagné la partie !\n\n**Stats de la partie :**\n__**Temps**__: ${time}\n__**Nombre de participants**__ : ${number}\n__**Participants**__ : \n${members}`,
            ...
            NUMBER_HIGHER: (number, author) =>
                `${author} | Le nombre est plus **grand** que \`${number}\` !`,
            NUMBER_SMALLER: (number, author) =>
                `${author} | Le nombre est plus **petit** que \`${number}\` !`,
            NUMBER_CONGRATS: member => `<@${member}> a gagné 10 crédits !`,
            NUMBER_STATS: (user, number, time, nb, members) =>
                `:tada: | ${user} a trouvé le nombre ! C'était __**${number}**__ !\n\n**Stats de la partie :**\n__**Temps**__: ${time}\n__**Nombre de participants**__ : ${nb}\n__**Participants**__ : \n${members}`,
            // Errors
            NUMBER_DEFEAT: number =>
                `${e.ERROR} | Personne n'a réussi à trouver le nombre ! C'était ${number} !`,

            /* RANDOM COMMAND */

            // Utils
            RANDOM_DESCRIPTION:
                "Tire aléatoirement un des choix que vous me donner !",
            ...
            QUOTE_EXAMPLES:
                "$quote 596018101921906698\n$quote 596018101921906698 573508780520898581\n$quote 596018101921906698 #blabla",
            // Errors
            QUOTE_ERR_NOT_FOUND: `${e.ERROR} | Aucun message ne possède cet ID.`,
            QUOTE_ERR_NOT_FOUND_CHANNEL: channel =>
                `${e.ERROR} | Aucun salon trouvé avec l'ID ${channel} !`,

            /* INVITATIONS COMMAND */

            // Utils
            INVITATIONS_DESCRIPTION:
                "Affiche le nombre de personnes que vous avez invitées sur le serveur !",
            INVITATIONS_USAGE: "invitations (@membre)",
            INVITATIONS_EXAMPLES: "$invitations\n$invitations @Androz#2091",
            // Errors
            INVITATIONS_ERR_NO_INVITE: member =>
                `${e.ERROR} | ${
                    member ? member.user.username + " n'a" : "Vous n'avez"
                } invité personne sur le serveur !`,
            // Content
            INVITATIONS_CODE: invite =>
                `**${invite.code}** (${invite.uses} utilisations) | ${invite.channel}`,
            // Headings
            INVITATIONS_TITLE: (member, msg) =>
                `Informations sur les invitations de ${member} sur ${msg.guild.name}`,
            ...
            ACHIEVEMENTS_DESCRIPTION:
                "Affiche la liste de toutes les réalisations !",
            ...
            ACHIEVEMENTS_PROGRESS: progressData =>
                `Progression : ${progressData.now}/${
                    progressData.total
                } (${Math.round(
                    100 * (progressData.now / progressData.total)
                )}%)`,
            ...
            // Utils
            SETAFK_DESCRIPTION:
                "Devenez AFK (les membres qui vous mentionneront recevront un message)",
            ...
            // Content
            SETAFK_SUCCESS: reason =>
                `${e.SUCCESS} | Vous êtes passé afk (raison : ${reason})`,
            // Others
            AFK_DELETED: `${e.WARN} | Votre AFK vient d'être supprimé !`,
            AFK_MEMBER: (user, reason) =>
                `${e.WARN} | **${user.tag}** est actuellement AFK pour :\n\`\`\`${reason}\`\`\``,
            ...
            REMINDME_FIELDS: ["Créé il y a", "Message"],
            ...
            // Errors
            USERINFO_ERR_ID: id =>
                `${e.ERROR} | Aucun utilisateur sur Discord ne possède l'ID \`${id}\` !`,
            // Content
            USERINFO_FIELDS: [
                ":man: Pseudo",
            ...
            USERINFO_LINKS: (discord, invite, github, website) =>
                `${
                    discord ? `[Support](https://discord.gg/${discord})\n` : ""
                }[Invitation](${invite})\n${
                    github ? `[Github](${github})\n` : ""
                }${website ? `[Site web](${website})` : ""}`,
            USERINFO_STATS: (votes, servers, shards, lib) =>
                `**${votes}** votes (Discordbots.org)\n**${servers}** serveurs\n**${
                    shards.length === 0 ? "Aucun" : shards.length
                }** shards\nDéveloppé avec **${lib}**`,
            ...
            SERVERINFO_MEMBERCOUNT: members =>
                `${members.filter(m => !m.user.bot).size} membres | ${
                    members.filter(m => m.user.bot).size
                } bots`,
            SERVERINFO_NO_AFK: "Aucun salon AFK",
            SERVERINFO_CHANNELS: channels =>
                `${channels.filter(ch => ch.type === "voice").size} vocaux | ${
                    channels.filter(ch => ch.type === "text").size
                } textuels | ${
                    channels.filter(ch => ch.type === "category").size
                } catégories`,

            /* MENTIONROLE COMMAND */

            // Utils
            MENTIONROLE_DESCRIPTION:
                "Mentionne un rôle puis désactive la possibilité de mentionner",
            ...
            UNBAN_ERR_ID: id =>
                `${e.ERROR} | Aucun utilisateur sur Discord ne possède l'ID \`${id}\` !`,
            UNBAN_ERR_NOT_BANNED: user =>
                `${e.ERROR} | **${user.username}** n'est pas banni !`,
            // Content
            UNBAN_SUCCESS: (user, msg) =>
                `${e.SUCCESS} | **${user.username}** vient d'être débanni de **${msg.guild.name}** !`,
            ...
            /* REPORT COMMAND */
            REPORT_DESCRIPTION:
                "Envoyez votre signalement dans le salon défini pour cela !",
            ...
            //Headings
            REPORT_HEADINGS: ["Auteur", "Date", "Contenu", "Signalement"],
            // Content
            REPORT_TITLE: user => `Signalement - ${user.tag}`,
            REPORT_SUCCESS: channel =>
                `${e.SUCCESS} | Votre rapport a été envoyé dans ${channel} !`,

            /* GETINVITE COMMAND */

            // Utils
            GETINVITE_DESCRIPTION:
                "Génère une invitation vers le serveur en question. Veillez à faire bon usage de cette commande.",
            GETINVITE_USAGE: "getinvite [ID/Nom]",
            GETINVITE_EXAMPLES:
                "$getinvite Atlanta\n$getinvite 565048515357835264",
            // Errors
            GETINVITE_ERR_NO_GUILD: search =>
                `${e.ERROR} | Aucun serveur trouvé (recherche: ${search})`,

            /* SUGGEST COMMAND */

            // Utils
            SUGGEST_DESCRIPTION:
                "Envoie votre suggestion dans le salon défini pour ça !",
            ...
            // Headings
            SUGGEST_HEADINGS: ["Auteur", "Date", "Contenu"],
            // Content
            SUGGEST_TITLE: user => `Suggestion - ${user.tag}`,
            SUGGEST_SUCCESS: channel =>
                `${e.SUCCESS} | Votre suggestion est en cours de vote dans ${channel} !`,
            ...
            INVITE_DESC: prefix =>
                `Tapez \`${prefix}invite copy\` pour pouvoir copier le lien !`,
            INVITE_HEADINGS: [
            ...
            MINECRAFT_DESCRIPTION:
                "Affiche des informations sur le serveur Minecraft !",
            ...
            // Utils
            STAFF_DESCRIPTION:
                "Affiche la liste des membres du staff du serveur !",
            STAFF_USAGE: "staff",
            ...
            FML_ERROR:
                "Une erreur est survenue pendant la récupération de la VDM",
            ...
            EIGHTBALL_EXAMPLES:
                "$8ball Est-ce qu'Atlanta est le meilleur bot Discord ?",
            // Errors
            ...
            APITOKEN_DESCRIPTION:
                "Renvoie votre token pour utiliser l'API d'Atlanta !",
            ...
            APITOKEN_SUCCESS: token =>
                `:key: | Votre clé d'API est \`${token}\` !`,
            APITOKEN_SUCCESS_REGENERATE: token =>
                `:key: | Clé d'API regénérée ! Votre clé d'API est maintenant \`${token}\`.`,

            /* LOVECALC COMMAND */

            // Utils
            LOVECALC_DESCRIPTION:
                "Combien d'amour y a t'il entre deux personnes ? *Ceci est une commande fun, à ne pas prendre au sérieux*",
            LOVECALC_USAGE: "lovecalc [@membre1] (@membre2)",
            LOVECALC_EXAMPLES:
                "$lovecalc @Androz#2091\n$lovecalc @Androz#2091 @Atlanta#6770",
            // Errors
            LOVECALC_ERR_MENTIONS: `${e.ERROR} | Vous devez mentionner deux membres !`,
            // Content
            LOVECALC_CONTENT: (percent, username1, username2) =>
                `Il y a **${percent}%** d'amour entre **${username1}** et **${username2}** !`,
            ...
            PERMISSIONS_DESCRIPTION:
                "Affiche les permissions du membre dans le salon",
            PERMISSIONS_USAGE: "permissions (@membre)",
            PERMISSIONS_EXAMPLES: "$permissions\n$permissions @Androz#2091",
            // Content
            PERMISSIONS_TITLE: (username, channel) =>
                `Permissions de ${username} dans #${channel}`,
            ...
            // Utils
            BACKUP_DESCRIPTION:
                "Gérez vos sauvegardes de serveur de manière ergonomique et efficace!",
            BACKUP_USAGE: "backup [create/load/infos]",
            BACKUP_EXAMPLES:
                "$backup create\n$backup load 92N1x\n$backup infos 92N1x",
            // Errors
            BACKUP_ERR_STATUS: `${e.ERROR} | Vous devez préciser \`create\`, \`load\` ou \`infos\` !`,
            BACKUP_ERR_NOT_FOUND: backupID =>
                `${e.ERROR} | Aucune sauvegarde trouvée pour \`${backupID}\``,
            ...
            BACKUP_CREATE_SUCCESS_ID: backupID =>
                `${e.SUCCESS} | Voici l'ID de votre sauvegarde : \`\`\`${backupID}\`\`\``,
            ...
            // Utils
            TWEET_DESCRIPTION:
                "Génère un tweet d'une personne de votre choix sur Twitter !",
            ...
            CONFIGURATION_HEADINGS: [
                ...
                [
                    "Auto-suppression commandes",
                    "Auto-suppression des commandes de modération désactivée"
                ],
                [
                    "Éditer votre configuration",
                    `[Cliquez ici pour accéder au dashboard !](${c.dashboard.baseURL})`
                ]
            ],
            CONFIGURATION_AUTOROLE: roleID => `Rôle : <@&${roleID}>`,
            CONFIGURATION_WELCOME: (withImage, channelID) =>
                `Salon : <#${channelID}>\nImage : ${withImage ? "Oui" : "Non"}`,
            CONFIGURATION_GOODBYE: (withImage, channelID) =>
                `Salon : <#${channelID}>\nImage : ${withImage ? "Oui" : "Non"}`,
            CONFIGURATION_MODLOGS: channelID =>
                `Logs modération : ${
                    channelID ? `<#${channelID}>` : "Indéfini"
                }`,
            CONFIGURATION_SUGGESTIONS: channelID =>
                `Suggestions : ${channelID ? `<#${channelID}>` : "Indéfini"}`,
            CONFIGURATION_REPORTS: channelID =>
                `Signalements : ${
                    channelID ? `<#${channelID}>` : "Non défini"
                }`,
            CONFIGURATION_FORTNITESHOP: channelID =>
                `Boutique Fortnite : ${
                    channelID ? `<#${channelID}>` : "Indéfini"
                }`,
            CONFIGURATION_AUTOMOD: ignoredChannels =>
                `${
                    ignoredChannels.length > 0
                        ? `Salon ignorés : ${ignoredChannels.map(
                              ch => `<#${ch}>`
                          )}`
                        : "Aucun salon ignoré."
                }`,
            CONFIGURATION_WARNS: (kick, ban) =>
                `${
                    kick
                        ? `**Expulsion**: au bout de **${kick}** avertissements.`
                        : "**Expulsion**: Non définie."
                }\n${
                    ban
                        ? `**Bannissement**: au bout de **${ban}** avertissements.`
                        : "**Bannissement**: Non défini."
                }`,
            CONFIGURATION_AUTODELETEMOD:
                "Auto-suppression des commandes de modération activée",

            /* IGNORE COMMAND */

            // Utils
            IGNORE_DESCRIPTION:
                "Désactive ou active les commandes dans le salon mentionné",
            IGNORE_USAGE: "ignore [#salon]",
            IGNORE_EXAMPLES: "$ignore #général",
            // Content
            IGNORE_SUCCESS_DISABLED: channel =>
                `${e.SUCCESS} | Les commandes sont maintenant autorisées dans ${channel} !`,
            IGNORE_SUCCESS_ENABLED: channel =>
                `${e.WARN} | Les commandes sont maintenant interdites dans ${channel} !`,
            ...
            // Content
            SETPREFIX_SUCCESS: prefix =>
                `${e.SUCCESS} | Le préfixe a bien été modifié ! Tapez \`${prefix}help\` pour voir la liste des commandes !`,

            /* AUTOROLE COMMAND */

            // Utils
            AUTOROLE_DESCRIPTION:
                "Active ou désactive l'autorôle sur le serveur !",
            ...
            // Content
            AUTOROLE_ENABLED: prefix =>
                `${e.SUCCESS} | Autôrole correctement activé ! Pour avoir plus d'informations sur la configuration de votre serveur tapez \`${prefix}configuration\` !`,
            AUTOROLE_DISABLED: prefix =>
                `${e.WARN} | Autôrole correctement désactivé ! Pour avoir plus d'informations sur la configuration de votre serveur tapez \`${prefix}configuration\` !`,
            ...
            WELCOME_DISABLED: prefix =>
                `${e.SUCCESS} | Les messages de bienvenue viennent d'être désactivés ! Tapez \`${prefix}configuration\` pour voir la configuration actuelle !`,
            WELCOME_FORM_CHANNEL: author =>
                `Bonjour ${author} ! Dans quel salon s'enverra le message de bienvenue ? (mentionnez un salon)`,
            WELCOME_FORM_MESSAGE: (channel, msg) =>
                `D'accord ! Les messages s'enverront donc dans ${channel}. Entrez le message de bienvenue ci-dessous : \n\nInfos:\`\`\`\nMention : {user}\nMembres : {membercount}\nServeur : {server}\`\`\`Par exemple, "Bienvenue {user} sur {server} ! Grâce à toi, nous sommes {membercount} !" donnera "Bienvenue ${msg.author} sur ${msg.guild.name} ! Grâce à toi, nous sommes ${msg.guild.memberCount} !".`,
            ...
            WELCOME_FORM_SUCCESS: (channel, prefix) =>
                `${e.SUCCESS} | Messages de bienvenue activés dans <#${channel}> ! Tapez \`${prefix}welcome test\` pour tester le message de bienvenue !`,
            ...
            WELCOME_DEFAULT_MESSAGE:
                "Bienvenue {user} sur {server} ! Grâce à toi, nous sommes {membercount} !",
            ...
            // Utils
            GOODBYE_DESCRIPTION:
                "Envoie un message d'au revoir dans un salon défini au préalable !",
            GOODBYE_USAGE: "goodbye",
            GOODBYE_EXAMPLES: "$goodbye",
            // Content
            GOODBYE_DISABLED: prefix =>
                `${e.SUCCESS} | Les messages d'au revoir viennent d'être désactivés ! Tapez \`${prefix}configuration\` pour voir la configuration actuelle !`,
            GOODBYE_TEST_SUCCESS: `${e.SUCCESS} | Test effectué !`,
            GOODBYE_FORM_CHANNEL: author =>
                `Bonjour ${author} ! Dans quel salon s'enverra le message d'au revoir ? (mentionnez un salon)`,
            GOODBYE_FORM_MESSAGE: (channel, msg) =>
                `D'accord ! Les messages s'enverront donc dans ${channel}. Entrez le message d'au revoir ci-dessous : \n\nInfos:\`\`\`\nMention : {user}\nMembres : {membercount}\nServeur : {server}\`\`\`Par exemple, "Au revoir {user} ! C'est triste, sans toi nous ne sommes que {membercount} sur {server} !" donnera "Au revoir ${msg.author.username}#${msg.author.discriminator} ! C'est triste, sans toi nous ne sommes que ${msg.guild.memberCount} sur ${msg.guild.name} !".`,
            ...
            GOODBYE_FORM_SUCCESS: (channel, prefix) =>
                `${e.SUCCESS} | Messages d'au revoir activés dans <#${channel}> ! Tapez \`${prefix}goodbye test\` pour tester le message d'au revoir !`,
            GOODBYE_IMG_MSG: name => `Départ de ${name}`,
            GOODBYE_IMG_NUMBER: memberCount => `- ${memberCount}ème membre !`,
            GOODBYE_IMG_TITLE: "AU REVOIR",
            GOODBYE_DEFAULT_MESSAGE:
                "Au revoir {user} ! C'est triste, sans toi nous ne sommes que {membercount} sur {server} !",
            // Errors
            ...
            // Errors
            SLOWMODE_PLEASE_WAIT: (time, channel) =>
                `${e.ERROR} | Le salon ${channel} est en slowmode ! Veuillez attendre ${time} pour pouvoir poster un nouveau message !`,
            // Content
            SLOWMODE_DISABLED: channel =>
                `${e.SUCCESS} | Le slowmode a été désactivé dans le salon <#${channel}> !`,
            SLOWMODE_ENABLED: (channel, time) =>
                `${e.SUCCESS} | Slowmode activé dans <#${channel}> avec un temps de ${time} !`,

            /* ADDCOMMAND COMMAND */

            // Utils
            ADDCOMMAND_DESCRIPTION:
                "Ajoutez une commande personnalisée au serveur !",
            ...
            ADDCOMMAND_ERR_EXISTS: name =>
                `${e.ERROR} | La commande ${name} existe déjà !`,
            ADDCOMMAND_ERR_ANSWER: `${e.ERROR} | Veuillez entrer une réponse à cette commande !`,
            // Content
            ADDCOMMAND_SUCCESS: cmd =>
                `${e.SUCCESS} | La commande ${cmd} a bien été ajoutée au serveur !`,

            /* DELCOMMAND COMMAND */

            // Utils
            DELCOMMAND_DESCRIPTION:
                "Enlevez une commande personnalisée du serveur !",
            ...
            DELCOMMAND_ERR_EXISTS: cmd =>
                `${e.ERROR} | La commande ${cmd} n'existe pas !`,
            // Content
            DELCOMMAND_SUCCESS: cmd =>
                `${e.SUCCESS} | La commande ${cmd} a bien été enlevée du serveur !`,
            ...
            RELOAD_ERR_NOT_FOUND: cmd =>
                `${e.ERROR} | Aucune commande trouvée pour \`${cmd}\` !`,
            // Content
            RELOAD_SUCCESS: cmd =>
                `${e.SUCCESS} | La commande ${cmd} a bien été reload !`,

            /* PROFILE COMMAND */

            // Utils
            PROFILE_DESCRIPTION:
                "Affiche le profil de l'utilisateur mentionné (ou de l'auteur du message)",
            ...
            PROFILE_ACHIEVEMENTS: prefix =>
                `Obtenez plus d'informations avec \`${prefix}achievements\` !`,
            ...
            WORK_EXAMPLES: "$work",
            // Content
            WORK_CLAIMED_HEADINGS: ["Salaire", "Série"],
            WORK_CLAIMED_SALARY: amount =>
                `${amount} crédits ajoutés à votre compte !`,
            WORK_AWARD: ":tada: Vous avez gagné 200 crédits bonus !",
            WORK_FOOTER:
                "Terminez le mot AWARD pour gagner 200 crédits bonus !",
            // Errors
            WORK_ERR_COOLDOWN: delay =>
                `${e.ERROR} | Vous devez attendre ${delay} avant de pouvoir de nouveau travailler !`,
            ...
            // Errors
            REP_ERR_COOLDOWN: delay =>
                `${e.ERROR} | Vous devez attendre ${delay} avant de pouvoir de nouveau donner un point de réputation !`,
            REP_ERR_YOURSELF: `${e.ERROR} | Vous ne pouvez pas vous donner vous-même un point de réputation !`,
            // Content
            REP_SUCCESS: tag =>
                `${e.SUCCESS} | Vous avez bien donné un point de réputation à **${tag}** !`,

            /* SETBIO COMMAND */

            // Utils
            SETBIO_DESCRIPTION:
                "Changez la description qui apparaitra sur votre profil !",
            ...
            LEADERBOARD_DESCRIPTION:
                "Affiche les utilisateurs qui dispose du plus de crédits, de niveaux ou de points de réputation !",
            ...
            ROB_ERR_AMOUNT: member =>
                `${e.ERROR} | Veuillez entrer un montant valide à piller à **${member.user.tag}** !`,
            ROB_ERR_AMOUNT_MEMBER: (member, money) =>
                `${e.ERROR} | **${member.user.username}** ne dispose pas de **${money}** crédits !`,
            ROB_ERR_NO_MONEY: needed =>
                `${e.ERROR} | Vous devez avoir plus de **${needed}** crédits pour tenter ce pillage !`,
            ROB_ERR_CLDWN: member =>
                `:spy: **${member.user.username}** est sur ses gardes... Patientez un peu et réessayez !`,
            // Content
            ...
            DEPOSIT_ERR_AMOUNT_TOO_HIGH: money =>
                `${e.ERROR} | Vous ne disposez pas de \`${money}\` crédits !`,
            // Content
            DEPOSIT_SUCCESS: money =>
                `${e.SUCCESS} | **${money}** crédits déposés à la banque !`,
            ...
            WITHDRAW_ERR_AMOUNT_TOO_HIGH: money =>
                `${e.ERROR} | Vous ne disposez pas de \`${money}\` crédits à la banque !`,
            // Content
            WITHDRAW_SUCCESS: money =>
                `${e.SUCCESS} | **${money}** crédits retirés !`,
            ...
            PAY_ERR_INVALID_AMOUNT: username =>
                `${e.ERROR} | Vous devez entrer un montant à verser à **${username}** !`,
            PAY_ERR_AMOUNT_TOO_HIGH: (amount, username) =>
                `${e.ERROR} | Vous ne disposez pas d'assez de crédits pour verser ${amount} crédits à ${username} !`,
            // Content
            PAY_SUCCESS: (amount, username) =>
                `${e.SUCCESS} | Vous avez versé ${amount} crédits à ${username} !`,

            /* BIRTHDATE COMMAND */

            // Utils
            BIRTHDATE_DESCRIPTION:
                "Définissez la date de votre anniversaire (qui apparaitre sur votre profil)",
            ...
            // Content
            BIRTHDATE_SUCCESS: date =>
                `${e.SUCCESS} | Votre date d'anniversaire a été définie sur le ${date} !`,

            /* WEDDING COMMAND */

            // Utils
            WEDDING_DESCRIPTION:
                "Mariez-vous avec la personne de votre choix !",
            WEDDING_USAGE: "wedding [@user#0000]",
            WEDDING_EXAMPLES: "$wedding @Androz#2091",
            // Errors
            WEDDING_ERR_AUTHOR_MARRIED: prefix =>
                `${e.ERROR} | Vous êtes déjà marié(e) ! Utilisez d'abord \`${prefix}divorce\` pour divorcer`,
            WEDDING_ERR_MEMBER_MARRIED: username =>
                `${e.ERROR} | La place est prise compagnon ! **${username}** est déjà marié(e) !`,
            WEDDING_ERR_AUTHOR_PENDING_REQUESTER: username =>
                `${e.ERROR} | Vous avez déjà une demande en cours auprès de **${username}** !`,
            WEDDING_ERR_AUTHOR_PENDING_RECEIVER: username =>
                `${e.ERROR} | **${username}** vous a déjà envoyé une demande ! Veuillez la refuser ou l'accepter (ou attendre qu'elle expire dans quelques minutes).`,
            WEDDING_ERR_MEMBER_PENDING_REQUESTER: (username1, username2) =>
                `${e.ERROR} | **${username2}** a déjà une demande envoyé une demande auprès de **${username1}** !`,
            WEDDING_ERR_MEMBER_PENDING_RECEIVER: (username1, username2) =>
                `${e.ERROR} | **${username1}** a déjà envoyé une demande auprès de **${username2}** ! Attendez que **${username2}** accepte ou refuse la demande de **${username1}** ou que celle-ci expire puis réessayez !`,
            WEDDING_ERR_TIMEOUT: member =>
                `${e.ERROR} | ${member} n'a pas répondu... Attendez qu'il/elle soit connecté(e) puis réessayez !`,
            WEDDING_ERR_DENIED: (author, member) =>
                `${e.ERROR} | ${author}, j'ai une mauvaise nouvelle... ${member} a refusé votre demande en mariage.`,
            WEDDING_ERR_YOURSELF: `${e.ERROR} | Vous ne pouvez pas vous épouser vous-même !`,
            // Content
            WEDDING_REQUEST: (member, author) =>
                `${e.WARN} | ${member}, acceptez-vous d'épouser ${author} ? Répondez par "oui" ou "non" !`,
            WEDDING_SUCCESS: (author, member) =>
                `${e.SUCCESS} | ${author}, j'ai une bonne nouvelle... ${member} a accepté votre demande en mariage !`,

            /* DIVORCE COMMAND */

            // Utils
            DIVORCE_DESCRIPTION:
                "Divorcez avec la personne avec qui vous êtes actuellement marié(e) !",
            DIVORCE_USAGE: "divorce",
            DIVORCE_EXAMPLES: "divorce",
            // Errors
            DIVORCE_ERR_NOT_WEDDED: `${e.ERROR} | Vous n'êtes actuellement pas marié(e) !`,
            // Content
            DIVORCE_SUCCESS: username =>
                `${e.SUCCESS} | Vous venez de divorcer avec **${username}** !`,
            ...
            // Content
            SLOTS_DEFEAT: (amount, username) =>
                `**${username}** a utilisé ${amount} crédit(s) et a tout perdu.`,
            SLOTS_VICTORY: (text, amount, won, username) =>
                `${text}**${username}** a utilisé ${amount} crédit(s) et a gagné ${won} crédit(s) !`,
            // Errors
            SLOTS_ERR_TOO_HIGH: money =>
                `${e.ERROR} | Vous ne disposez pas de ${money} crédit(s).`,
            ...
            STATS_CREDITS:
                'Merci à `https://icones8.fr/icons/`, tous les emojis (ou presque) viennent de ce site !\n__**Donateurs**__:\n- `xixi52#0001` **GOD**\n- `Marty#3994` **GOD**\n- `"</Synaro/>"#2019` **GOD**\n- `🍮_Lucas_🍮#6171` **SUPPORTER**',
            STATS_LINKS: (url, id) =>
                `[Donate](${c.others.donate}) ● [Invite](https://discordapp.com/oauth2/authorize?client_id=${id}&scope=bot&permissions=2146958847) ● [Serveur](${url}) ● [Github](${c.others.github})`,
            // Headings
            STATS_HEADINGS: [
                `Stats`,
            ...
            // Utils
            FIRE_DESCRIPTION:
                'Génère une image de type "fire" grâce à l\'API Améthyste',
            ...
            // Utils
            JAIL_DESCRIPTION:
                'Génère une image de type "jail" grâce à l\'API Améthyste',
            ...
            // Utils
            MISSION_DESCRIPTION:
                'Génère une image de type "mission" grâce à l\'API Améthyste',
            ...
            // Utils
            RIP_DESCRIPTION:
                'Génère une image de type "rip" grâce à l\'API Améthyste',
            ...
            // Utils
            SCARY_DESCRIPTION:
                'Génère une image de type "scary" grâce à l\'API Améthyste',
            ...
            // Utils
            TOBECONTINUED_DESCRIPTION:
                'Génère une image de type "tobecontinued" grâce à l\'API Améthyste',
            TOBECONTINUED_USAGE: "tobecontinued (@membre)",
            TOBECONTINUED_EXAMPLES:
            ...
            TRIGGERED_DESCRIPTION:
                'Génère une image de type "triggered" grâce à l\'API Améthyste',
            ...
            // Utils
            WANTED_DESCRIPTION:
                'Génère une image de type "wanted" grâce à l\'API Améthyste',
            ...
            // Utils
            WASTED_DESCRIPTION:
                'Génère une image de type "wasted" grâce à l\'API Améthyste',
            ...
            // Utils
            CAPTCHA_DESCRIPTION:
                'Génère une image de type "captcha" grâce à l\'API Nekobot',
            ...
            // Utils
            PHCOMMENT_DESCRIPTION:
                'Génère une image de type "phcomment" grâce à l\'API Nekobot',
            ...
            // Utils
            LOVE_DESCRIPTION:
                'Génère une image de type "love" grâce à l\'API Nekobot',
            ...
            // Utils
            CLYDE_DESCRIPTION:
                'Génère une image de type "clyde" grâce à l\'API Nekobot',
            CLYDE_USAGE: "clyde [texte]",
            CLYDE_EXAMPLES:
                "$clyde Discord va fermer le 11 novembre 2019. Au revoir.",
            // Errors
            ...
            // Content
            PLAY_ADDED_TO_QUEUE: title =>
                `${e.add} | ${title} a été ajouté à la queue !`,
            PLAY_SEARCH:
            ...
            STOP_CONTENT: (voteCount, requiredCount) =>
            ...
            SKIP_CONTENT: (title, voteCount, requiredCount) =>
                `Chanson suivante : ${title}\nVote : ${voteCount}/${requiredCount}\nRéagissez avec 👍 pour passer à la chanson suivante !`,
            SKIP_CONTENT_COMPLETE: title =>
                `Chanson passée ! Maintenant : ${title}`,
            // Errors
            ...
            // Utils
            NP_DESCRIPTION:
                "Affiche des informations sur la chanson actuelle !",
            NP_USAGE: "np",
            NP_EXAMPLES: "$np",
            // Errors
            ...
            LYRICS_ERR_NO_LYRICS: songName =>
                `${e.ERROR} | Aucune parole trouvée pour \`${songName}\` !`,
            ...
            // Errors
            TRANSLATE_ERR_LANG: prefix =>
                `${e.ERROR} | Veuillez entrer une langue ! Pour afficher la liste des langues, tapez \`${prefix}translate langs-list\` !`,
            TRANSLATE_ERR_NOT_FOUND: (prefix, lang) =>
                `${e.ERROR} | La langue \`${lang}\` n'existe pas ! Pour afficher la liste des langues, tapez \`${prefix}translate langs-list\` !`,
            ...
            // Errors
            BAN_ERR_BANNED: user =>
                `${e.ERROR} | **${user.username}** est déjà banni !`,
            ...
            BAN_SUCCESS_DM: (user, msg, reason) =>
                `${e.ERROR} | Bonjour <@${user.id}>,\nVous venez d'être banni de **${msg.guild.name}** par **${msg.author.tag}** pour **${reason}** !`,
            BAN_SUCCESS_CHANNEL: (user, msg, reason) =>
                `${e.SUCCESS} | **${user.username}** vient d'être banni de **${msg.guild.name}** par **${msg.author.tag}** pour **${reason}** !`,
            ...
            KICK_SUCCESS_DM: (user, msg, reason) =>
                `${e.ERROR} | Bonjour <@${user.id}>,\nVous venez d'être expulsé de **${msg.guild.name}** par **${msg.author.tag}** pour **${reason}** !`,
            KICK_SUCCESS_CHANNEL: (user, msg, reason) =>
                `${e.SUCCESS} | **${user.username}** vient d'être expulsé de **${msg.guild.name}** par **${msg.author.tag}** pour **${reason}** !`,
            KICK_TITLE_LOGS: caseNumber => `Kick | Cas #${caseNumber}`,

            /* CHECKINVITES COMMAND */

            // Utils
            CHECKINVITES_DESCRIPTION:
                "Vérifie si les membres n'ont pas une publicité pour leur serveur Discord dans leur présence !",
            ...
            CLEAR_SUCCESS: amount =>
                `${e.SUCCESS} | **${amount}** messages supprimés !`,
            CLEAR_SUCCESS_USER: (amount, user) =>
                `${e.SUCCESS} | **${amount}** messages de **${user.tag}** supprimés !`,

            /* MUTE COMMAND */

            // Utils
            MUTE_DESCRIPTION:
                "Empêche le membre d'envoyer des messages et de se connecter en vocal pendant un certain temps !",
            MUTE_USAGE: "mute [@membre] [temps]",
            MUTE_EXAMPLES: "$mute @Androz#2091 Spam",
            // Content
            MUTE_SUCCESS: (member, time, reason) =>
                `${e.SUCCESS} | **${member.user.tag}** est maintenant mute pendant **${time}** pour **${reason}** !`,
            MUTE_SUCCESS_DM: (message, time, reason) =>
                `${e.ERROR} | Vous êtes mute sur **${message.guild.name}** pendant **${time}** pour **${reason}** !`,
            ...
            // Content
            UNMUTE_SUCCESS: (userID, caseNumber) =>
                `<@${userID}> vient d'être unmute ! (cas du mute : #${caseNumber})`,
            UNMUTE_SUCCESS_USER: user =>
                `${e.SUCCESS} | ${user.tag} vient d'être unmute !`,
            ...
            // Content
            WARN_AUTOBAN: (member, number) =>
                `${e.SUCCESS} | **${member.user.tag}** a été banni automatiquement car il avait plus de **${number}** warns !`,
            WARN_AUTOKICK: (member, number) =>
                `${e.SUCCESS} | **${member.user.tag}** a été expulsé automatiquement car il avait plus de **${number}** warns !`,
            WARN_SUCCESS_DM: (msg, reason) =>
                `${e.WARN} | Vous venez d'être averti sur **${msg.guild.name}** par **${msg.author.tag}** pour **${reason}** !`,
            WARN_SUCCESS: (member, reason) =>
                `${e.SUCCESS} | **${member.user.tag}** a été averti par messages privés pour **${reason}** !`,
            WARN_TITLE_LOGS: caseNumber => `Avertissement | Cas #${caseNumber}`,

            /* CLEARSANCTIONS COMMAND */

            // Utils
            CLEARSANCTIONS_DESCRIPTION:
            ...
            // Utils
            SETWARNS_DESCRIPTION:
                "Définissez les sanctions qu'obtiendront les membres au bout d'un certain nombre de warns !",
            ...
            // Content
            SETWARNS_SUCCESS_KICK: (prefix, number) =>
                `${e.SUCCESS} | Configuration enregistrée ! Lorsqu'un membre aura atteint les ${number} warns, il sera expulsé. Tapez \`${prefix}configuration\` pour voir votre nouvelle configuration !`,
            SETWARNS_SUCCESS_BAN: (prefix, number) =>
                `${e.SUCCESS} | Configuration enregistrée ! Lorsqu'un membre aura atteint les ${number} warns, il sera banni. Tapez \`${prefix}configuration\` pour voir votre nouvelle configuration !`,
            SETWARNS_SUCCESS_RESET_KICK: prefix =>
                `${e.SUCCESS} | Configuration enregistrée ! La sanction kick n'est maintenant plus automatique ! Tapez \`${prefix}configuration\` pour voir votre nouvelle configuration !`,
            SETWARNS_SUCCESS_RESET_BAN: prefix =>
                `${e.SUCCESS} | Configuration enregistrée ! La sanction ban n'est maintenant plus automatique ! Tapez \`${prefix}configuration\` pour voir votre nouvelle configuration !`,
            ...
            // Content
            POLL_FORM_MENTION:
                'Souhaitez-vous ajouter une mention à votre message ? Répondez par "oui" ou "non" !',
            POLL_FORM_MENTION_HE:
                "Tapez une des réponses suivantes : `every` (pour une mention @ everyone) ou `here` (pour une mention @ here) !",
            ...
            ANNOUNCEMENT_DESCRIPTION:
                "Envoie une annonce dans le salon actuel !",
            ...
            // Content
            ANNOUNCEMENT_FORM_MENTION:
                'Souhaitez-vous ajouter une mention à votre annonce ? Répondez par "oui" ou "non" !',
            ANNOUNCEMENT_FORM_MENTION_HE:
                "Tapez une des réponses suivantes : `every` (pour une mention @ everyone) ou `here` (pour une mention @ here) !",
            ...
            GIVEAWAY_REROLL_MESSAGES: {
                congrat:
                    ":tada: Nouveau gagnant(s) : {winners} ! Félicitations !",
                error:
            ...
            SANCTIONS_DESCRIPTION:
                "Affiche la liste des infractions commises par un membre !",
            ...
            PRINT_SANCTION: sData =>
                `Modérateur: <@${sData.moderator}>\nRaison: ${sData.reason}`,

            /* DELETEMOD COMMAND */

            // Utils
            DELETEMOD_DESCRIPTION:
                "Active ou désactive l'auto suppression des commandes de modération !",
            ...
            // Utils
            SETMODLOGS_DESCRIPTION:
                "Définissez le salon des logs de modération !",
            SETMODLOGS_USAGE: "setmodlogs (#salon)",
            SETMODLOGS_EXAMPLES: "$setmodlogs #modlogs\n$setmodlogs",
            // Content
            SETMODLOGS_SUCCESS: id =>
                `${e.SUCCESS} | Salon des logs de modération défini sur <#${id}> !`,
            ...
            // Headings
            SOMEONE_HEADINGS: ["Pseudo", "Discriminant", "ID"],
            ...
            // Content
            SETSUGGESTS_SUCCESS: channel =>
                `${e.SUCCESS} | Le salon des suggestions est maintenant ${channel} !`,
            ...
            // Content
            SETREPORTS_SUCCESS: channel =>
                `${e.SUCCESS} | Le salon des signalements est maintenant ${channel} !`,
            ...
            ADDEMOTE_EXAMPLES:
                "$addemote https://une-image-de.com/papillon.png papillon",
            // Errors
            ...
            // Content
            ADDEMOTE_SUCCESS: emote =>
                `${e.SUCCESS} | Émoji ${
                    emote.name
                } ajouté au serveur ! Chaîne : \`${emote.toString()}\``,

            /* AUTOMOD COMMAND */

            // Utils
            AUTOMOD_DESCRIPTION:
                "Active ou désactive la suppression automatique des invitations discord",
            ...
            // Content
            AUTOMOD_SUCCESS_ENABLED: prefix =>
                `${e.SUCCESS} | Les invitations Discord seront automatiquement supprimées ! Si vous souhaitez ignorer un salon, tapez simplement \`${prefix}automod off #salon\` ! Cela désactivera l'auto modération dans le salon mentionné !`,
            AUTOMOD_SUCCESS_DISABLED_CHANNEL: channel =>
                `${e.SUCCESS} | L'auto modération ne sera plus effectuée dans le salon ${channel} !`,
            AUTOMOD_SUCCESS_DISABLED: `${e.SUCCESS} | Très bien ! L'auto modération n'est plus effective sur ce serveur !`,
            AUTOMOD_MSG: msg =>
                `${msg.author} | Votre message contenait une invitation Discord, il a donc été supprimé. Si c'était involontaire, vous pouvez rééditer votre message, il vous a été envoyé en message privé !`,
            ...
            FORTNITE_ERR_NOT_FOUND: (platform, username) =>
                `${e.ERROR} | Joueur \`${username}\` introuvable sur la plateforme \`${platform}\` !`,
            // Content
            FORTNITE_TITLE: (username, link) =>
                `Stats Fortnite de [${username}](${link})`,
            ...
            // Utils
            SETFORTNITESHOP_DESCRIPTION:
                "Définissez le salon de la boutique Fortnite du jour !",
            SETFORTNITESHOP_USAGE: "setfortniteshop (#salon)",
            SETFORTNITESHOP_EXAMPLES:
                "$setfortniteshop #shop\n$setfortniteshop",
            // Content
            SETFORTNITESHOP_DISABLED: `${e.SUCCESS} | Boutique Fortnite du jour désactivée !`,
            SETFORTNITESHOP_ENABLED: id =>
                `${e.SUCCESS} | Salon de la boutique Fortnite du jour défini sur <#${id}> !`,

            /* CALC COMMAND */

            // Utils
            CALC_DESCRIPTION:
                "Une calculatrice capable de résoudre des opérations complexes et convertir des unités !",
            ...
            // Content
            PURGE_CONFIRMATION: members =>
                `${e.WARN} | ${members} membres seront expulsés ! Pour confirmer, tapez \`confirm\` !`,
            PURGE_SUCCESS: members =>
                `${e.SUCCESS} | ${members} membres expulsés !`,

            /* DASHBOARD */

            FIRST_LOGIN: username =>
                `${username} s'est connecté pour la première fois au dashboard ! :tada:`,
            ...
            ERR_NOT_FOUND_CONTENT:
                "Nous n'avons pas trouvé la page que vous cherchiez. En attendant, vous pouvez retourner au tableau de bord ou essayer d'utiliser le formulaire de recherche.",
            ERR_SOMETING_WENT_WRONG: "Oups ! Quelque chose s'est mal passé.",
            ERR_SOMETING_WENT_WRONG_CONTENT:
                "Nous nous efforcerons d'y remédier tout de suite. En attendant, vous pouvez retourner au tableau de bord ou essayer d'utiliser le formulaire de recherche.",
            ERR_NO_SERVER_FOUND: "Aucun serveur trouvé",
            ERR_NO_SERVER_FOUND_CONTENT:
                "Aucun serveur à afficher. Vérifiez que vous êtes connecté avec le bon compte et réessayez.",
            ...
            SETTINGS_HEADINGS: ["🇫🇷 Langue", "Déconnexion"]
        };
    }
    ...
    printDate(pdate, isLongDate) {
        ...
        let hour =
            pdate.getHours() < 10 ? "0" + pdate.getHours() : pdate.getHours();
        let minute =
            pdate.getMinutes() < 10
                ? "0" + pdate.getMinutes()
                : pdate.getMinutes();
