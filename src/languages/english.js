...
module.exports = class {
    constructor() {
        this.language = {
            ...
            VOTE_THANKS: user =>
                `:arrow_up: Hello ${user.toString()}, thanks for voting !\nYour reward : 40 credits !`,
            VOTE_LOGS: user =>
                `:arrow_up: **${user.tag}** (\`${user.id}\`) voted for **Atlanta** and won **40** credits, thank you!\nhttps://discordbots.org/bot/557445719892688897/vote`,
            ...
            ERR_INVALID_NUMBER: nan =>
                `${e.ERROR} | \`${nan}\` is not a valid number!`,
            ERR_INVALID_NUMBER_MM: (min, max) =>
                `${e.ERROR} Please indicate a valid number between ${min} and ${max}!`,
            ...
            ERR_MISSING_BOT_PERMS: perms =>
                `${e.ERROR} | I need the following permissions to perform this command: \`${perms}\``,
            ERR_MISSING_MEMBER_PERMS: perm =>
                `${e.ERROR} | You do not have the necessary permissions to perform this command (\`${perm}\`)`,
            ...
            ERR_UNAUTHORIZED_CHANNEL: channel =>
                `${e.ERROR} | Commands are forbidden in ${channel} !`,
            ERR_BAD_PARAMETERS: (cmd, prefix) =>
                `${e.ERROR} | Please check the commands parameters. Look at the examples by typing \`${prefix}help ${cmd}\` !`,
            ERR_ROLE_NOT_FOUND: role =>
                `${e.ERROR} | No role found with \`${role}\` !`,
            ERR_CHANNEL_NOT_FOUND: channel =>
                `${e.ERROR} | No channel found with \`${channel}\``,
            ...
            ERR_CMD_COOLDOWN: seconds =>
                `${e.ERROR} | You must wait **${seconds}** second(s) to be able to run this command again!`,
            ...
            // Utils
            HELP_DESCRIPTION:
                "Displays the help of commands or the help of a particular command",
            HELP_USAGE: "help (command)",
            HELP_EXAMPLES: "$help\n$help ping",
            // Errors
            HELP_ERR_NOT_FOUND: cmd =>
                `${e.ERROR} | Unable to find command \`${cmd}\`!`,
            HELP_ERR_CMD_CUSTOMIZED: cmd =>
                `${e.ERROR} | The \`${cmd}\` command has no help because it is customized.`,
            // Content
            HELP_EDESCRIPTION: prefix =>
                `● To get help on an command type \`${prefix}help <command>\`!`,
            ...
            FINDWORDS_DESCRIPTION:
                "Start a game of findwords, a game where you have to find words!",
            FINDWORDS_USAGE: "findwords",
            FINDWORDS_EXAMPLES: "$findwords",
            // Errors
            FINDWORDS_ERR_INVALID_WORD: member =>
                `${e.ERROR} | ${member} your word is invalid!`,
            ...
            FINDWORDS_QUESTION: word =>
                `${e.WARN} | 20 seconds to find a word containing "**${word}**" !`,
            FINDWORDS_CONGRATS: winner =>
                `${e.SUCCESS} | Well done <@${winner}> ! Your word is valid and you were the fastest!`,
            FINDWORDS_STATS: (username, games, time, number, members) =>
                `:tada: | ${username} won the game !\n\n**Stats of the game: **\n__**Time**__: ${time}\n__**Number of participants**__ : ${number}\n__**Participants**__ : \n${members}`,
            ...
            NUMBER_HIGHER: (number, author) =>
                `${author} | The number is more **large** than \`${number}\` !`,
            NUMBER_SMALLER: (number, author) =>
                `${author} | The number is more **small** thane \`${number}\` !`,
            NUMBER_CONGRATS: member => `<@${member}> has won 10 credits!`,
            NUMBER_STATS: (user, number, time, nb, members) =>
                `:tada: | ${user} found the number! It was __**${number}**__ !\n\n\n**States of the game: **\n__**Time**__: ${time}\n__** Number of participants**__ : ${nb}\n__**Participants**__ : \n${members}`,
            // Errors
            NUMBER_DEFEAT: number =>
                `${e.ERROR} | No one could find the number! It was ${number} !`,
            ...
            QUOTE_ERR_NOT_FOUND_CHANNEL: channel =>
                `${e.ERROR} | No channel found with ID ${channel} !`,

            /* INVITATIONS COMMAND */

            // Utils
            INVITATIONS_DESCRIPTION:
                "Displays the number of people you have invited to the server!",
            ...
            INVITATIONS_ERR_NO_INVITE: member =>
                ...
                } didn't invite anyone to the server!`,
            // Content
            INVITATIONS_CODE: invite =>
                `**${invite.code}** (${invite.uses} uses) | ${invite.channel}`,
            // Headings
            INVITATIONS_TITLE: (member, msg) =>
                `Information on the invitations to ${member} on ${msg.guild.name}`,
            ...
            ACHIEVEMENTS_DESCRIPTION:
                "Displays the list of all the achievements!",
            ...
            // Utils
            SETAFK_DESCRIPTION:
                "Become an AFK (members who mention you will receive a message)",
            ...
            SETAFK_SUCCESS: reason =>
                `${e.SUCCESS} | You passed afk (reason: ${reason})`,
            // Others
            AFK_DELETED: `${e.WARN} | Your AFK has just been deleted!`,
            AFK_MEMBER: (user, reason) =>
                `${e.WARN} | **${user.tag}** is currently AFK for:\n\`\`\`${reason}\`\`\``,
            ...
            REMINDME_FIELDS: ["Created", "Message"],
            ...
            // Errors
            USERINFO_ERR_ID: id =>
                `${e.ERROR} | No user on Discord has the ID \`${id}\` !`,
            // Content
            USERINFO_FIELDS: [
                ":man: Pseudo",
            ...
            USERINFO_STATS: (votes, servers, shards, lib) =>
                `**${votes}** votes (Discordbots.org)\n**${servers}** servers\n**${
                    shards.length === 0 ? "No" : shards.length
                }** shards\nMade with **${lib}**`,
            ...
            MENTIONROLE_DESCRIPTION:
                "Mention a role and then disable the possibility of mentioning",
            ...
            UNBAN_ERR_NOT_BANNED: user =>
                `${e.ERROR} | **${user.username}** is not banned!`,
            // Content
            UNBAN_SUCCESS: (user, msg) =>
                `${e.SUCCESS} | **${user.username}** has just been unbanned from **${msg.guild.name}** !`,
            ...
            /* REPORT COMMAND */
            REPORT_DESCRIPTION:
                "Send your report to the channel defined for this!",
            ...
            //Headings
            REPORT_HEADINGS: ["Author", "Date", "Content", "Reporting"],
            // Content
            REPORT_TITLE: user => `Report - ${user.tag}`,
            REPORT_SUCCESS: channel =>
                `${e.SUCCESS} | Your report has been sent in ${channel} !`,

            /* GETINVITE COMMAND */

            // Utils
            GETINVITE_DESCRIPTION:
                "Generates an invitation to the server in question. Make sure you make good use of this command.",
            GETINVITE_USAGE: "getinvite [ID/Name]",
            GETINVITE_EXAMPLES:
                "$getinvite Atlanta\n$getinvite 565048515357835264",
            // Errors
            GETINVITE_ERR_NO_GUILD: search =>
                `${e.ERROR} | No server found (search: ${search})`,

            /* SUGGEST COMMAND */

            // Utils
            SUGGEST_DESCRIPTION:
                "Send your suggestion to the channel defined for this!",
            ...
            // Headings
            SUGGEST_HEADINGS: ["Author", "Date", "Content"],
            // Content
            SUGGEST_TITLE: user => `Suggestion - ${user.tag}`,
            SUGGEST_SUCCESS: channel =>
                `${e.SUCCESS} | Your suggestion is being voted on in ${channel} !`,
            ...
            INVITE_DESC: prefix =>
                `Type \`${prefix}invite copy\` to be able to copy the link!`,
            INVITE_HEADINGS: [
            ...
            MINECRAFT_DESCRIPTION:
                "Displays information about the Minecraft server!",
            ...
            APITOKEN_DESCRIPTION:
                "Send your token back to use the Atlanta API!",
            ...
            APITOKEN_SUCCESS_REGENERATE: token =>
                `:key: | Regenerated API key! Your API key is now \`${token}\`.`,

            /* LOVECALC COMMAND */

            // Utils
            LOVECALC_DESCRIPTION:
                "How much love is there between two people? *This is a fun command, not to be taken seriously*",
            ...
            LOVECALC_CONTENT: (percent, username1, username2) =>
                `There's **${percent}%** of love between **${username1}** and **${username2}** !`,

            /* BACKUP COMMAND */

            // Utils
            BACKUP_DESCRIPTION:
                "Manage your server backups in an ergonomic and efficient way!",
            ...
            BACKUP_ERR_NOT_FOUND: backupID =>
                `${e.ERROR} | No backup found for \`${backupID}\``,
            ...
            BACKUP_CREATE_SUCCESS_ID: backupID =>
                `${e.SUCCESS} | Here is the ID of your backup: \`\`\`${backupID}\`\`\``,
            ...
            PERMISSIONS_DESCRIPTION:
                "Displays the member's permissions in the channel",
            ...
            PERMISSIONS_TITLE: (username, channel) =>
                `Permissions of ${username} in #${channel}`,
            ...
            SERVERSLIST_DESCRIPTION:
                "Displays the list of the servers of the bot!",
            ...
            // Utils
            TWEET_DESCRIPTION:
                "Generate a tweet of a person of your choice on Twitter!",
            ...
            PLAY_ADDED_TO_QUEUE: title =>
                `${e.add} | ${title} has been added to the queue!`,
            PLAY_SEARCH:
                "Please enter a value to select one of the search results from 1 to 10.",
            ...
            PLAY_SUCCESS: song => `▶️ Currently playing ${song.name}...`,
            PLAY_ADDED_TO_QUEUE: song => `🎵 ${song.name} added to the queue!`,
            ...
            // Utils
            PAUSE_DESCPRIPTION: "Pause the current song",
            PAUSE_USAGE: "pause",
            ...
            // Utils
            RESUME_DESCPRIPTION: "Resume the current song",
            RESUME_USAGE: "resume",
            ...
            LYRICS_ERR_NO_LYRICS: songName =>
                `${e.ERROR} | No lyrics found for \`${songName}\` !`,
            ...
            CONFIGURATION_HEADINGS: [
                ...
                [
                    ...
                    "Auto-deletion of moderation commands disabled"
                ],
                [
                    "Edit your configuration",
                    `[Clic here to go on the dashboard!](${c.dashboard.baseURL})`
                ]
            ],
            ...
            CONFIGURATION_MODLOGS: channelID =>
                `Moderation logs : ${
                    channelID ? `<#${channelID}>` : "Not defined"
                }`,
            ...
            CONFIGURATION_REPORTS: channelID =>
                `Reports : ${channelID ? `<#${channelID}>` : "Not defined"}`,
            ...
            CONFIGURATION_WARNS: (kick, ban) =>
                `${
                    kick
                        ? `**Expulsion**: after **${kick}** warnings.`
                        : "**Expulsion**: Not defined."
                }\n${
                    ban
                        ? `**Banishment**: after **${ban}** warnings.`
                        : "**Banishment**: Not defined."
                }`,
            CONFIGURATION_AUTODELETEMOD:
                "Auto-deletion of moderation commands enabled",

            /* IGNORE COMMAND */

            // Utils
            IGNORE_DESCRIPTION:
                "Disables or activates commands in the mentioned channel",
            IGNORE_USAGE: "ignore [#channel]",
            IGNORE_EXAMPLES: "$ignore #general",
            // Content
            IGNORE_SUCCESS_DISABLED: channel =>
                `${e.SUCCESS} | Commands are now allowed in ${channel} !`,
            IGNORE_SUCCESS_ENABLED: channel =>
                `${e.WARN} | Commands are now forbidden in ${channel} !`,
            ...
            SETPREFIX_SUCCESS: prefix =>
                `${e.SUCCESS} | The prefix has been changed! Type in \`${prefix}help\` to see the list of commands!`,

            /* AUTOROLE COMMAND */

            // Utils
            AUTOROLE_DESCRIPTION:
                "Enable or disable the autorole on the server!",
            ...
            AUTOROLE_ENABLED: prefix =>
                `${e.SUCCESS} | Autrole correctly activated! To have more information about the configuration of your server type \`${prefix}configuration\` !`,
            AUTOROLE_DISABLED: prefix =>
                `${e.WARN} | Autrole correctly deactivated! To have more information about the configuration of your server type \`${prefix}configuration\` !`,
            ...
            WELCOME_DISABLED: prefix =>
                `${e.SUCCESS} | Welcome messages have just been disabled! Type \`${prefix}configuration\` to see the current configuration!`,
            WELCOME_FORM_CHANNEL: author =>
                `Hello ${author} ! In which channel will the welcome messages be sent? (mention a channel)`,
            WELCOME_FORM_MESSAGE: (channel, msg) =>
                `All right! The messages will therefore be sent in ${channel}. Enter the welcome message below: \n\n\nInfos:\n\n\nMention: {user}\nMembers: {membercount}\nServer: {server}\nFor example, "Welcome {user} to {server} ! Thanks to you, we are {membercount} ! will give "Welcome ${msg.author} to ${msg.guild.name} ! Thanks to you, we are ${msg.guild.memberCount} !".`,
            ...
            WELCOME_FORM_SUCCESS: (channel, prefix) =>
                `${e.SUCCESS} | Welcome messages enabled in <#${channel}> ! Type \`${prefix}welcome test\` to test the welcome message!`,
            ...
            WELCOME_DEFAULT_MESSAGE:
                "Welcome {user} to {server} ! Thanks to you, we are {membercount} !",
            // Errors
            ...
            GOODBYE_FORM_SUCCESS: (channel, prefix) =>
                `${e.SUCCESS} | Goodbye messages enabled in <#${channel}> ! Type \`${prefix}goodbye test\` to test the goodbye message!`,
            ...
            GOODBYE_DEFAULT_MESSAGE:
                "Goodbye {user} ! It's sad, without you we're only {membercount} on {server} !",
            // Errors
            ...
            SLOWMODE_PLEASE_WAIT: (time, channel) =>
                `${e.ERROR} | The channel ${channel} is in slowmode! Please wait ${time} to be able to post a new message!`,
            // Content
            SLOWMODE_DISABLED: channel =>
                `${e.SUCCESS} | The slowmode has been disabled in the channel <#${channel}> !`,
            SLOWMODE_ENABLED: (channel, time) =>
                `${e.SUCCESS} | Slowmode enabled in <#${channel}> with a time of ${time} !`,
            ...
            ADDCOMMAND_ERR_EXISTS: name =>
                `${e.ERROR} | The command ${name} already exists!`,
            ADDCOMMAND_ERR_ANSWER: `${e.ERROR} | Please enter an answer to this command!`,
            // Content
            ADDCOMMAND_SUCCESS: cmd =>
                `${e.SUCCESS} | The command ${cmd} has been added to the server!`,
            ...
            DELCOMMAND_ERR_EXISTS: cmd =>
                `${e.ERROR} | The command ${cmd} does not exist!`,
            // Content
            DELCOMMAND_SUCCESS: cmd =>
                `${e.SUCCESS} | The ${cmd} command has been removed from the server!`,
            ...
            RELOAD_ERR_NOT_FOUND: cmd =>
                `${e.ERROR} | No command found for \`${cmd}\` !`,
            // Content
            RELOAD_SUCCESS: cmd =>
                `${e.SUCCESS} | The command ${cmd} has been reloaded!`,

            /* PROFILE COMMAND */

            // Utils
            PROFILE_DESCRIPTION:
                "Displays the profile of the mentioned user (or the author of the message)",
            ...
            PROFILE_ACHIEVEMENTS: prefix =>
                `Get more information with \`${prefix}achievements\`!`,
            ...
            WORK_EXAMPLES: "$work",
            // Content
            WORK_CLAIMED_HEADINGS: ["Salary", "Streak"],
            ...
            WORK_ERR_COOLDOWN: delay =>
                `${e.ERROR} | You have to wait ${delay} before you can work again!`,
            ...
            REP_ERR_COOLDOWN: delay =>
                `${e.ERROR} | You have to wait ${delay} before you can give a reputation point again!`,
            REP_ERR_YOURSELF: `${e.ERROR} | You can't give yourself a reputation point!`,
            // Content
            REP_SUCCESS: tag =>
                `${e.SUCCESS} | You did give a reputation point to **${tag}** !`,

            /* SETBIO COMMAND */

            // Utils
            SETBIO_DESCRIPTION:
                "Change the biography that will appear on your profile!",
            ...
            LEADERBOARD_DESCRIPTION:
                "Displays users who have the most credits, levels or reputation points!",
            ...
            ROB_ERR_AMOUNT: member =>
                `${e.ERROR} | Please enter a valid amount to be robbed to **${member.user.tag}** !`,
            ROB_ERR_AMOUNT_MEMBER: (member, money) =>
                `${e.ERROR} | **${member.user.username}** does not have **${money}** credits !`,
            ROB_ERR_NO_MONEY: needed =>
                `${e.ERROR} | You must have more than **${needed}** credits to attempt this robbery!`,
            ROB_ERR_CLDWN: member =>
                `:spy: **${member.user.username}** is on guard.... Wait a while and try again!`,
            // Content
            ...
            DEPOSIT_ERR_AMOUNT_TOO_HIGH: money =>
                `${e.ERROR} | You don't have \`${money}\` credits!`,
            // Content
            DEPOSIT_SUCCESS: money =>
                `${e.SUCCESS} | **${money}** credits deposited in the bank!`,
            ...
            PAY_ERR_INVALID_AMOUNT: username =>
                `${e.ERROR} | You must enter an amount to be paid to **${username}** !`,
            PAY_ERR_AMOUNT_TOO_HIGH: (amount, username) =>
                `${e.ERROR} | You don't have enough credits to pay ${amount} credits to ${username} !`,
            // Content
            PAY_SUCCESS: (amount, username) =>
                `${e.SUCCESS} | You paid ${amount} credits to ${username} !`,
            ...
            WITHDRAW_ERR_AMOUNT_TOO_HIGH: money =>
                `${e.ERROR} | You do not have \`${money}\` credits at the bank!`,
            // Content
            WITHDRAW_SUCCESS: money =>
                `${e.SUCCESS} | **${money}** credits withdrawn!`,

            /* BIRTHDATE COMMAND */

            // Utils
            BIRTHDATE_DESCRIPTION:
                "Set your birthday date (which will appear on your profile)",
            ...
            BIRTHDATE_SUCCESS: date =>
                `${e.SUCCESS} | Your birthday has been set on the ${date} !`,
            ...
            WEDDING_ERR_AUTHOR_MARRIED: prefix =>
                `${e.ERROR} | You are already married! First use \`${prefix}divorce\` to divorce`,
            WEDDING_ERR_MEMBER_MARRIED: username =>
                `${e.ERROR} | The place is taken, companion! **${username}** is already married!`,
            WEDDING_ERR_AUTHOR_PENDING_REQUESTER: username =>
                `${e.ERROR} | You already have a current request to **${username}** !`,
            WEDDING_ERR_AUTHOR_PENDING_RECEIVER: username =>
                `${e.ERROR} | **${username}** has already sent you a request! Please refuse or accept it (or wait until it expires in a few minutes).`,
            WEDDING_ERR_MEMBER_PENDING_REQUESTER: (username1, username2) =>
                `${e.ERROR} | **${username2}** has already sent a request to **${username1}** !`,
            WEDDING_ERR_MEMBER_PENDING_RECEIVER: (username1, username2) =>
                `${e.ERROR} | **has already sent a request to **${username2}** ! Wait until **${username2}** accepts or refuses the request for **${username1}** or until it expires and try again!`,
            WEDDING_ERR_TIMEOUT: member =>
                `${e.ERROR} | ${member} did not answer.... Wait until he/she is connected and then try again!`,
            WEDDING_ERR_DENIED: (author, member) =>
                `${e.ERROR} | ${author}, I have some bad news... ${member} refused your proposal.`,
            WEDDING_ERR_YOURSELF: `${e.ERROR} | You can't marry yourself!`,
            // Content
            WEDDING_REQUEST: (member, author) =>
                `${e.WARN} | ${member}, do you agree to marry ${author}? Answer with "yes" or "no"!`,
            WEDDING_SUCCESS: (author, member) =>
                `${e.SUCCESS} | ${author}, I have some good news... ${member} has accepted your proposal!`,

            /* DIVORCE COMMAND */

            // Utils
            DIVORCE_DESCRIPTION:
                "Divorce the person you are currently married to!",
            ...
            DIVORCE_SUCCESS: username =>
                `${e.SUCCESS} | You just divorced with **${username}** !`,
            ...
            SLOTS_DEFEAT: (amount, username) =>
                `**${username}** used ${amount} credit(s) and lost everything.`,
            SLOTS_VICTORY: (text, amount, won, username) =>
                `${text}**${username}** used ${amount} credit(s) and won ${won} credit(s)!`,
            // Errors
            SLOTS_ERR_TOO_HIGH: money =>
                `${e.ERROR} | You do not have ${money} credit(s).`,
            ...
            STATS_CREDITS:
                'Thanks to `https://icones8.fr/icons/`, all the emojis (or almost) come from this site!\n__**Donators**__:\n- `xixi52#0001` **GOD**\n- `Marty#3994` **GOD**\n- `"</Synaro/>"#2019` **GOD**\n- `🍮_Lucas_🍮#6171` **SUPPORTER**',
            STATS_LINKS: (url, id) =>
                `[Donate](${c.others.donate}) ● [Invite](https://discordapp.com/oauth2/authorize?client_id=${id}&scope=bot&permissions=2146958847) ● [Server](${url}) ● [Github](${c.others.github})`,
            // Headings
            STATS_HEADINGS: [
                `Stats`,
            ...
            // Utils
            FIRE_DESCRIPTION:
                'Generates a "fire" image using the Améthyste API',
            ...
            // Utils
            JAIL_DESCRIPTION:
                'Generates a "jail" image using the Améthyste API',
            ...
            // Utils
            MISSION_DESCRIPTION:
                'Generates a "mission" image using the Améthyste API',
            ...
            // Utils
            SCARY_DESCRIPTION:
                'Generates a "scary" image using the Améthyste API',
            ...
            TOBECONTINUED_DESCRIPTION:
                'Generates a "tobecontinued" image using the Améthyste API',
            ...
            TRIGGERED_DESCRIPTION:
                'Generates a "triggered" image using the Améthyste API',
            ...
            // Utils
            WANTED_DESCRIPTION:
                'Generates a "wanted" image using the Améthyste API',
            ...
            // Utils
            WASTED_DESCRIPTION:
                'Generates a "wasted" image using the Améthyste API',
            ...
            // Utils
            CAPTCHA_DESCRIPTION:
                'Generates a "triggered" image using the Nekobot API',
            ...
            PHCOMMENT_DESCRIPTION:
                'Generates a "phcomment" image using the Nekobot API',
            ...
            // Utils
            CLYDE_DESCRIPTION:
                'Generates a "clyde" image using the Nekobot API',
            CLYDE_USAGE: "clyde [text]",
            CLYDE_EXAMPLES:
                "$clyde Discord will close on November 11, 2019. Goodbye.",
            // Errors
            ...
            TRANSLATE_ERR_LANG: prefix =>
                `${e.ERROR} | Please enter a language! To display the list of languages, type \`${prefix}translate langs-list\` !`,
            TRANSLATE_ERR_NOT_FOUND: (prefix, lang) =>
                `${e.ERROR} | The language \`${lang}\` does not exist! To display the list of languages, type \`${prefix}translate langs-list\` !`,
            ...
            // Errors
            BAN_ERR_BANNED: user =>
                `${e.ERROR} | **${user.username}** is already banned!`,
            ...
            BAN_SUCCESS_DM: (user, msg, reason) =>
                `${e.ERROR} | Hello <@${user.id}>,\nYou have just been banned from **${msg.guild.name}** by **${msg.author.tag}** for **${reason}** !`,
            BAN_SUCCESS_CHANNEL: (user, msg, reason) =>
                `${e.SUCCESS} | **${user.username}** has just been banned from **${msg.guild.name}** by **${msg.author.tag}** for **${reason}** !`,
            ...
            KICK_SUCCESS_CHANNEL: (user, msg, reason) =>
                `${e.SUCCESS} | **${user.username}** has just been kicked out from **${msg.guild.name}** by **${msg.author.tag}** for **${reason}** !`,
            ...
            CHECKINVITES_DESCRIPTION:
                "Check if members do not have an ad for their Discord server in their presence!",
            ...
            CLEAR_SUCCESS: amount =>
                `${e.SUCCESS} | **${amount}** messages deleted!`,
            CLEAR_SUCCESS_USER: (amount, user) =>
                `${e.SUCCESS} | **${amount}** messages of **${user.tag}** deleted !`,

            /* MUTE COMMAND */

            // Utils
            MUTE_DESCRIPTION:
                "Prevents the member from sending messages and connecting by voice for a period of time!",
            MUTE_USAGE: "mute [@member] [time]",
            MUTE_EXAMPLES: "$mute @Androz#2091 Spam",
            // Content
            MUTE_SUCCESS: (member, time, reason) =>
                `${e.SUCCESS} | **${member.user.tag}** is now muted for **${time}** for **${reason}** !`,
            MUTE_SUCCESS_DM: (message, time, reason) =>
                `${e.ERROR} | You are muted in **${message.guild.name}** for **${time}** for **${reason}** !`,
            ...
            UNMUTE_SUCCESS: (userID, caseNumber) =>
                `<@${userID}> has just been unmuted! (case of mute: #${caseNumber})`,
            UNMUTE_SUCCESS_USER: user =>
                `${e.SUCCESS} | ${user.tag} has just been unmuted!`,

            /* SANCTIONS COMMAND */

            // Utils
            SANCTIONS_DESCRIPTION:
                "Displays the list of infractions committed by a member!",
            SANCTIONS_USAGE: "sanctions [@member]",
            SANCTIONS_EXAMPLES: "$sanctions @Androz#2091",
            // Errors
            SANCTIONS_ERR_NOTHING:
                "This member has not committed any infractions.",
            PRINT_SANCTION: sData =>
                `Moderator: <@${sData.moderator}>\nReason: ${sData.reason}`,

            /* DELETEMOD COMMAND */

            // Utils
            DELETEMOD_DESCRIPTION:
                "Enables or disables the auto deletion of moderation commands!",
            ...
            WARN_AUTOBAN: (member, number) =>
                `${e.SUCCESS} | **${member.user.tag}** was automatically banned because it had more than **${number}** warns !`,
            WARN_AUTOKICK: (member, number) =>
                `${e.SUCCESS} | **${member.user.tag}** was automatically kicked out because he had more than **${number}** warns !`,
            WARN_SUCCESS_DM: (msg, reason) =>
                `${e.WARN} | You've just been warned on **${msg.guild.name}** by **${msg.author.tag}** for **${reason}** !`,
            WARN_SUCCESS: (member, reason) =>
                `${e.SUCCESS} | **${member.user.tag}** has been warned by private messages for **${reason}** !`,
            ...
            SETWARNS_DESCRIPTION:
                "Define the sanctions that members will get after a certain number of warns!",
            ...
            SETWARNS_SUCCESS_KICK: (prefix, number) =>
                `${e.SUCCESS} | Configuration saved! When a member has reached the ${number} warns, he will be kicked out. Type \`${prefix}configuration\` to see your new configuration!`,
            SETWARNS_SUCCESS_BAN: (prefix, number) =>
                `${e.SUCCESS} | Configuration saved! When a member has reached the ${number} warns, he will be banned. Type \`${prefix}configuration\` to see your new configuration!`,
            SETWARNS_SUCCESS_RESET_KICK: prefix =>
                `${e.SUCCESS} | Configuration saved! The kick sanction is no longer automatic! Type \`${prefix}configuration\` to see your new configuration!`,
            SETWARNS_SUCCESS_RESET_BAN: prefix =>
                `${e.SUCCESS} | Configuration saved! The ban sanction is no longer automatic! Type \`${prefix}configuration\` to see your new configuration!`,
            ...
            // Content
            POLL_FORM_MENTION:
                'Would you like to add a mention to your message? Answer "yes" or "no"!',
            POLL_FORM_MENTION_HE:
                "Type one of the following answers: `every` (for a mention @ everyone) or `here` (for a mention @ here)!",
            ...
            ANNOUNCEMENT_DESCRIPTION:
                "Send an announcement in the current channel!",
            ...
            SETMODLOGS_SUCCESS: id =>
                `${e.SUCCESS} | Moderation logs channel defined on <#${id}> !`,
            ...
            // Headings
            SOMEONE_HEADINGS: ["Pseudo", "Discriminator", "ID"],
            ...
            SETSUGGESTS_SUCCESS: channel =>
                `${e.SUCCESS} | The suggestions channel is now ${channel} !`,
            ...
            SETREPORTS_SUCCESS: channel =>
                `${e.SUCCESS} | The reports channel is now ${channel} !`,
            ...
            ADDEMOTE_EXAMPLES:
                "$addemote https://an-image-of.com/butterfly.png butterfly",
            // Errors
            ...
            ADDEMOTE_SUCCESS: emote =>
                `${e.SUCCESS} | Emoji ${
                    emote.name
                } added to the server! String: \`${emote.toString()}\``,

            /* AUTOMOD COMMAND */

            // Utils
            AUTOMOD_DESCRIPTION:
                "Enables or disables automatic deletion of discord invitations",
            ...
            AUTOMOD_SUCCESS_ENABLED: prefix =>
                `${e.SUCCESS} | Discord invitations will be automatically deleted! If you want to ignore a channel, just type \`${prefix}automod off #channel\` ! This will disable auto moderation in the mentioned channel!`,
            AUTOMOD_SUCCESS_DISABLED_CHANNEL: channel =>
                `${e.SUCCESS} | Automoderation will no longer be performed in the channel ${channel} !`,
            AUTOMOD_SUCCESS_DISABLED: `${e.SUCCESS} | All right! Auto moderation is no longer effective on this server!`,
            AUTOMOD_MSG: msg =>
                `${msg.author} | Your message contained a Discord invitation, so it has been deleted. If it was unintentional, you can edit your message again, it was sent to you as a private message!`,
            ...
            FORTNITE_ERR_NOT_FOUND: (platform, username) =>
                `${e.ERROR} | Player \`${username}\` not found on the platform \`${platform}\`!`,
            // Content
            FORTNITE_TITLE: (username, link) =>
                `[${username}](${link})'s Fortnite Stats`,
            FORTNITE_STATS_RIGHT: (kd, percent) =>
                `${kd} K/D - ${percent} WIN%`,
            ...
            SETFORTNITESHOP_DESCRIPTION:
                "Define the daily Fortnite shop channel!",
            ...
            SETFORTNITESHOP_ENABLED: id =>
                `${e.SUCCESS} | Fortnite shop channel defined on <#${id}> !`,

            /* CALC COMMAND */

            // Utils
            CALC_DESCRIPTION:
                "A calculator capable of solving complex operations and converting units!",
            ...
            PURGE_CONFIRMATION: members =>
                `${e.WARN} | ${members} members will be kicked out! To confirm, type \`confirm\`!`,
            PURGE_SUCCESS: members =>
                `${e.SUCCESS} | ${members} membres kicked !`,

            /* DASHBOARD */

            FIRST_LOGIN: username =>
                `${username} connected for the first time to the dashboard! :tada:`,
            ...
            ERR_NOT_FOUND_CONTENT:
                "We did not find the page you were looking for. In the meantime, you can go back to the dashboard or try using the search form.",
            ERR_SOMETING_WENT_WRONG: "Oops! Something went wrong.",
            ERR_SOMETING_WENT_WRONG_CONTENT:
                "We will try to remedy this immediately. In the meantime, you can go back to the dashboard or try using the search form.",
            ...
            SETTINGS_HEADINGS: ["🇬🇧 Language", "Logout"]
        };
    }
