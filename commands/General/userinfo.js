...
class Userinfo extends Command {
	...
	async run(message, args, data) {
		...
		const embed = new Discord.MessageEmbed()
			.setAuthor({ name: user.tag, iconURL: user.displayAvatarURL({ size: 512, dynamic: true, format: "png" }) })
		...
		if (member) {
			embed.addField(this.client.customEmojis.games + " " + message.translate("common:GAME"), (member.presence.activities ? member.presence.activities[0].name : message.translate("general/userinfo:NO_GAME")), true)
				.addField(this.client.customEmojis.status.online + " " + message.translate("common:STATUS"), message.translate("common:STATUS_" + (member.presence.status.toUpperCase())), true)
				.addField(this.client.customEmojis.up + " " + message.translate("common:ROLE"), `${member.roles.highest ? member.roles.highest : message.translate("general/userinfo:NO_ROLE")}`, true)
