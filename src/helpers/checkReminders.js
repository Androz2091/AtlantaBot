...
module.exports = {
    ...
    init(client) {
        setInterval(async function() {
            ...
            let users = await client.usersData
                .find({ reminds: { $gt: [] } })
                .lean();
            ...
            users.forEach(async user => {
                ...
                if (dUser) {
                    ...
                    if (reminds.length > 0) {
                        ...
                        if (mustSent.length > 0) {
                            mustSent.forEach(r => {
                                let embed = new Discord.MessageEmbed()
                                    ...
                                    .addField(
                                        language.get("REMINDME_FIELDS")[0],
                                        language.convertMs(
                                            dateNow - r.createdAt
                                        )
                                    )
                                    .addField(
                                        language.get("REMINDME_FIELDS")[1],
                                        r.message
                                    )
                        ...
                        let mUser = await client.usersData.findOne({
                            id: user.id
                        });
                        mUser.reminds = user.reminds.filter(
                            r => r.sendAt >= dateNow
                        );
                        mUser.save();
                    }
