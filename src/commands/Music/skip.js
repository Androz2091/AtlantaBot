...
module.exports = class extends Command {
    ...
    async execute(message, args) {
        ...
        if(votedUsers.includes(message.author.id)) return message.error("music/skip:ALREADY_VOTED");
        ...
        const voteCountNeeded = userVoiceCount === 1 ? 1 : Math.ceil(userVoiceCount/2);
        if(votedUsers.length == voteCountNeeded){
            delete votes[message.guild.id];
            player.stop();
            const skipEmbed = new Discord.MessageEmbed()
            .setDescription(message.translate("music/skip:SKIPPED", {
            ...
            .setDescription(message.translate("music/skip:VOTE_NEEDED", {
                current: votedUsers.length,
                needed: voteCountNeeded,
                users: channel.members.filter((m) => !m.user.bot).map((m) => votedUsers.includes(m.id) ? message.translate("music/skip:USER_VOTED", {
                    username: m.toString()
                }) : m.toString()).join("\n")
            }))
            .defaultColor();
