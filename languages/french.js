...
module.exports = class {
    constructor(...args) {
		this.language = {
			...
			YES_OR_NO: `${e.error} | Vous devez répondre par "oui" ou par "non" !`,
			INVALID_TIME: `${e.error} | Vous devez entrer un temps valide ! Unités valides : \`s\`, \`m\`, \`h\`, \`d\`, \`w\`, \`y\``,
			MENTION_EVERYONE: `${e.error} | Vous n'avez pas l'autorisation de mentionner everyone ou here dans les commandes.`,
			IS_A_BOT: `${e.error} | Cet utilisateur est un bot !`,
			NAN: (nan) => `${e.error} | \`${nan}\` n'est pas un nombre valide !`,
			INVALID_ID: `${e.error} | Veuillez entrer une ID valide !`,
			PLEASE_WAIT: `${e.loading} | Veuillez patienter...`,
			GAME_ALREADY_LAUNCHED: `${e.error} | Une partie est déjà en cours sur ce serveur !`,
			A_GAME_ALREADY_LAUNCHED: `${e.error} | A cause des lags et bugs dus au findwords et au number, il est impossible de lancer deux parties en même temps, même si elles sont sur deux serveurs différents.\nIl y a une partie actuellement en cours sur un autre serveur, veuillez donc patientez quelques minutes puis réessayer.\nNous sommes désolés, mais des personnes abusaient de cette commande en la spammant sur pleins de serveurs.`,
			AN_ERROR_OCCURENCED: `${e.error} | Une erreur est survenue, veuillez réessayez dans quelques minutes.`,
			NUMBER_1_10: `${e.error} Veuillez indiquer un nombre valide entre 1 et 10 !`,
