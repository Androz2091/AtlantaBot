...
module.exports = class extends Command {
    ...
    async execute(message, args) {
        ...
        if(!args[0] || isNaN(ms(args[0]))) return message.error("music/seek:MISSING", null, false, true);
        const amountSent = ms(args[0]);
        const amount = amountSent >= 602000 ? 602000 : amountSent <= 0 ? 0 : amountSent;
        if((player.position + amount) > player.queue[0].duration) return message.error("music/seek:CANNOT_SEEK", {
            time: this.client.helpers.convertTime.execute(message.guild, amount)
        });
        player.seek(player.position + amount);
        const seekEmbed = new Discord.MessageEmbed()
        .setDescription(message.translate("music/seek:SEEKED", {
            time: this.client.helpers.convertTime.execute(message.guild, amount)
        }, false, true))
